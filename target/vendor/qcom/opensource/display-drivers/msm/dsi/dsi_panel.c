...
static int dsi_panel_gpio_request(struct dsi_panel *panel)
{
	...
	if (gpio_is_valid(r_config->lcd_enp_en_gpio)) {
		rc = gpio_request(r_config->lcd_enp_en_gpio, "lcd_enp_en_gpio");
		if (rc) {
			DSI_ERR("request for lcd_enp_en_gpio failed, rc=%d\n", rc); 
			goto error;
		}
	}

	if (gpio_is_valid(r_config->lcd_enn_en_gpio)) {
		rc = gpio_request(r_config->lcd_enn_en_gpio, "lcd_enn_en_gpio");
		if (rc) {
			DSI_ERR("request for lcd_enn_en_gpio failed, rc=%d\n", rc); 
			goto error;
		}
	}
...
static int dsi_panel_gpio_release(struct dsi_panel *panel)
{
	...
	struct dsi_panel_reset_config *r_config = &panel->reset_config;
        ...
        if (gpio_is_valid(r_config->lcd_enp_en_gpio))
		gpio_free(r_config->lcd_enp_en_gpio);

        if (gpio_is_valid(r_config->lcd_enn_en_gpio))
		gpio_free(r_config->lcd_enn_en_gpio);
...
static int dsi_panel_power_on(struct dsi_panel *panel)
{
	...
	if (rc) {
       ...
       if (gpio_is_valid(panel->reset_config.lcd_tp_en_gpio))
                gpio_set_value(panel->reset_config.lcd_tp_en_gpio, 0);
       ...
       if (gpio_is_valid(panel->reset_config.tp_reset_gpio)) {
                ...
                if (rc) {
                        DSI_ERR("unable to set dir for tp_reset_gpio rc=%d\n", rc);
                        goto exit;
                }
        }

       if (gpio_is_valid(panel->reset_config.lcd_enp_en_gpio)) {
                rc = gpio_direction_output(panel->reset_config.lcd_enp_en_gpio, 1);
                if (rc) {
                        DSI_ERR("unable to set dir for lcd_enp_en_gpio rc=%d\n", rc);
                        goto exit;
                }
        }
       ...
       if (gpio_is_valid(panel->reset_config.lcd_enn_en_gpio)) {
                rc = gpio_direction_output(panel->reset_config.lcd_enn_en_gpio, 1);
                if (rc) {
                        DSI_ERR("unable to set dir for lcd_enn_en_gpio rc=%d\n", rc);
                        goto exit;
                }
        }
...
static int dsi_panel_power_off(struct dsi_panel *panel)
{
	...
	if (panel->is_twm_en || panel->skip_panel_off) {
  	...
  	if (gpio_is_valid(panel->reset_config.tp_reset_gpio))
		gpio_set_value(panel->reset_config.tp_reset_gpio, 0);
        usleep_range(3000,3500);
	if (gpio_is_valid(panel->reset_config.disp_en_gpio))
		gpio_set_value(panel->reset_config.disp_en_gpio, 0);
	...
	if (gpio_is_valid(panel->reset_config.lcd_enn_en_gpio))
		gpio_set_value(panel->reset_config.lcd_enn_en_gpio, 0);

	if (gpio_is_valid(panel->reset_config.lcd_enp_en_gpio))
		gpio_set_value(panel->reset_config.lcd_enp_en_gpio, 0);
        usleep_range(3000,3500);

        if (gpio_is_valid(panel->reset_config.lcd_tp_en_gpio)) {
                rc = gpio_direction_output(panel->reset_config.lcd_tp_en_gpio, 1);
                if (rc) {
                        DSI_ERR("unable to set dir for lcd_tp_en_gpio rc=%d\n", rc);
                }
        }
	/*if (gpio_is_valid(panel->reset_config.lcd_mode_sel_gpio))
...
static int mipi_dsi_dcs_set_display_dimming(struct mipi_dsi_device *dsi, u8 format)
{
	ssize_t err;
	err = mipi_dsi_dcs_write(dsi, MIPI_DCS_WRITE_CONTROL_DISPLAY, &format,
				 sizeof(format));
	if (err < 0)
		return err;
	return 0;
}
static int dsi_panel_update_dimming(struct dsi_panel *panel,
	u32 dimming)
{
	int rc = 0;
	unsigned long mode_flags = 0;
	...
	rc = mipi_dsi_dcs_set_display_dimming(dsi, dimming);
	if (rc < 0)
		DSI_ERR("failed to update dcs dimming:%d\n", dimming);
...
int dsi_panel_set_backlight(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	if (panel->host_config.ext_bridge_mode)
        ...
        if (last_bl_lvl != 0 && bl_lvl != 0 && dimming == 1) {
            dsi_panel_update_dimming(panel, 0x2C);
            dimming = 0;
        }
        last_bl_lvl = bl_lvl;
        bl_lvl = ((bl_lvl * 2559) / 4095) + 1;
        if (last_bl_lvl == 0 ) {
            dimming = 1;
        }
        if(bl_lvl < 50) {
            value = (bl_lvl)*545/49+3550;
            bl_lvl = 50;
            lcd_bright_i2c_set(value);
        } else {
            lcd_bright_i2c_set(4095);
        }
...
static int dsi_panel_parse_gpios(struct dsi_panel *panel)
{
	...
	char *reset_gpio_name, *mode_set_gpio_name;
        char *lcd_enp_en_gpio_name, *lcd_enn_en_gpio_name, *lcd_tp_en_gpio_name, *tp_reset_gpio_name;

	if (!strcmp(panel->type, "primary")) {
		reset_gpio_name = "qcom,platform-reset-gpio";
		mode_set_gpio_name = "qcom,panel-mode-gpio";
                lcd_enp_en_gpio_name = "qcom,platform-enp-en-gpio";
                lcd_enn_en_gpio_name = "qcom,platform-enn-en-gpio";
                lcd_tp_en_gpio_name = "qcom,platform-tp-en-gpio";
	...
	panel->reset_config.lcd_enp_en_gpio = utils->get_named_gpio(utils->data,
					      lcd_enp_en_gpio_name, 0);
	if (!gpio_is_valid(panel->reset_config.lcd_enp_en_gpio)) {
		rc = panel->reset_config.lcd_enp_en_gpio;
		DSI_ERR("[%s] failed get oled_disp_vio_en_gpio, rc=%d\n", panel->name, rc);
		goto error;
	}

	panel->reset_config.lcd_enn_en_gpio = utils->get_named_gpio(utils->data,
					      lcd_enn_en_gpio_name, 0);
	if (!gpio_is_valid(panel->reset_config.lcd_enn_en_gpio)) {
		rc = panel->reset_config.lcd_enn_en_gpio;
		DSI_ERR("[%s] failed get reset gpio, rc=%d\n", panel->name, rc);
		goto error;
	}

        panel->reset_config.lcd_tp_en_gpio = utils->get_named_gpio(utils->data,
					      lcd_tp_en_gpio_name, 0);
	if (!gpio_is_valid(panel->reset_config.lcd_tp_en_gpio)) {
		rc = panel->reset_config.lcd_tp_en_gpio;
		DSI_ERR("[%s] failed get reset gpio, rc=%d\n", panel->name, rc);
		goto error;
	}
	...
	if (!gpio_is_valid(panel->reset_config.tp_reset_gpio)) {
		rc = panel->reset_config.tp_reset_gpio;
		DSI_ERR("[%s] failed get tp_reset_gpio, rc=%d\n", panel->name, rc);
		goto error;
	}
