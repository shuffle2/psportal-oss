...
const char * g_pcb_name = NULL;
static bool  g_is_evt0 = false;
static struct power_supply *bat_psy;
static union power_supply_propval pwr_val = {0, };
/******************************************************
 *
 * Marco
 *
...
#define LED_WORK_TIMER 1
static void aw210xx_ledswork_start(struct aw210xx *aw210xx);
#if LED_WORK_TIMER
static struct timer_list  leds_work_timer;
static struct aw210xx * g_awledstimer;
static struct work_struct ledseffect_work;
static int led_breatheffect_time =  500;
static int g_leds_poweroffchg_mode = 0;
#endif

static unsigned char leds_timer_status = 0;
/******************************************************
 *
...
static int aw210xx_hw_enable(struct aw210xx *aw210xx, bool flag)
{
	...
	if (aw210xx && gpio_is_valid(aw210xx->enable_gpio)) {
		if (flag) {
			gpio_set_value_cansleep(aw210xx->sel_gpio, 1);
			gpio_set_value_cansleep(aw210xx->enable_gpio, 1);
			AW_LOG("aw210xx->enable_gpio is %d,sel gpio:%d\n",gpio_get_value(aw210xx->enable_gpio),
				gpio_get_value(aw210xx->sel_gpio));
			usleep_range(2000, 2500);
		} else {
			gpio_set_value_cansleep(aw210xx->sel_gpio, 0);
		...
		struct device_attribute *attr, char *buf)
{
	...
	len += snprintf(buf + len, PAGE_SIZE - len, "hwen=%d, sel gpio=%d\n",
			gpio_get_value(aw210xx->enable_gpio), gpio_get_value(aw210xx->sel_gpio));
	return len;
}

void ledseffect_work_func(struct work_struct *led_test)
{
#if LED_WORK_TIMER
	static int led_brightness_num = 0, effect_flag = 0, bat_soc=0;
	...
	if (bat_psy) {
		if (!power_supply_get_property(bat_psy,
			POWER_SUPPLY_PROP_CAPACITY, &pwr_val)) {
			bat_soc = pwr_val.intval;
		}
	}

	if(bat_soc < 95 ) {
		led_breatheffect_time = 500;
	} else if  (bat_soc >= 95) {
		led_breatheffect_time = 100;
	} else {
	}

	aw210xx_i2c_write(g_awledstimer, AW210XX_REG_GCR,0x01);
	aw210xx_i2c_write(g_awledstimer, AW210XX_REG_UVCR,0x00);
	aw210xx_i2c_write(g_awledstimer, AW210XX_REG_UVCR+1,0x00);

    if(effect_flag == 0) {
        if(led_brightness_num < led_breatheffect_time+1) {
			aw210xx_i2c_write(g_awledstimer,AW210XX_REG_BR00L+2*0,    ((0x7f*led_brightness_num)/led_breatheffect_time));//rgb1-g
			aw210xx_i2c_write(g_awledstimer,AW210XX_REG_BR00L+2*1,    0);//rgb1-b
			aw210xx_i2c_write(g_awledstimer,AW210XX_REG_BR00L+2*2,   ((0xff*led_brightness_num)/led_breatheffect_time));//rgb1-r

			aw210xx_i2c_write(g_awledstimer,AW210XX_REG_BR00L+2*3,   ((0xff*led_brightness_num)/led_breatheffect_time));//rgb2-r
			aw210xx_i2c_write(g_awledstimer,AW210XX_REG_BR00L+2*4,  ((0x7f*led_brightness_num)/led_breatheffect_time));//rgb2-g
			aw210xx_i2c_write(g_awledstimer,AW210XX_REG_BR00L+2*5,0x00);//rgb2-b

			aw210xx_i2c_write(g_awledstimer,AW210XX_REG_UPDATE,0x00);//
            led_brightness_num++;

                if(led_brightness_num == led_breatheffect_time+1) {
                    effect_flag = 1;
					led_brightness_num = led_breatheffect_time;
				}
        }
    }

    if(effect_flag == 1) {
        if(led_brightness_num > 0) {
			aw210xx_i2c_write(g_awledstimer,AW210XX_REG_BR00L+2*0,    ((0x7f*led_brightness_num)/led_breatheffect_time));//rgb1-g
			aw210xx_i2c_write(g_awledstimer,AW210XX_REG_BR00L+2*1,    0);//rgb1-b
			aw210xx_i2c_write(g_awledstimer,AW210XX_REG_BR00L+2*2,   ((0xff*led_brightness_num)/led_breatheffect_time));//rgb1-r
...
void leds_breath_func(struct timer_list *unused)
{
	#if LED_WORK_TIMER
		leds_work_timer.expires = jiffies + HZ/100;
		schedule_work(&ledseffect_work);
		mod_timer(&leds_work_timer, leds_work_timer.expires);
	#else
		aw210xx_i2c_write(g_awledstimer,AW210XX_REG_BR00L+2*4,((0x7f*2)/led_breatheffect_time));//rgb2-g
		AW_ERR("led timer test\n");
		leds_work_timer.expires = jiffies + HZ*2;
		mod_timer(&leds_work_timer, leds_work_timer.expires);
	#endif
}

static ssize_t aw210xx_ledswork_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t len)
{
	struct led_classdev *led_cdev = dev_get_drvdata(dev);
	...
	if (val > 0 && leds_timer_status == 0) {
		aw210xx_hw_enable(aw210xx, true);
		aw210xx_chipen_set(aw210xx, true);
		AW_ERR("-------------------------------- ADD TIMER --------------------------------\n");

		aw210xx_i2c_write(aw210xx,AW210XX_REG_GCCR,0xff);
		aw210xx_i2c_write(aw210xx,AW210XX_REG_SL00+0,0xff);//rgb1-g
		aw210xx_i2c_write(aw210xx,AW210XX_REG_SL00+1,0x00);//rgb1-b
		aw210xx_i2c_write(aw210xx,AW210XX_REG_SL00+2,0xff);//rgb1-r

		aw210xx_i2c_write(aw210xx,AW210XX_REG_SL00+3,0xff);//rgb2-r
		aw210xx_i2c_write(aw210xx,AW210XX_REG_SL00+4,0xff);//rgb2-g
		aw210xx_i2c_write(aw210xx,AW210XX_REG_SL00+5,0x00);//rgb2-b

        AW_ERR("[awled] led breath effect timer start, aw210xx pointer: %p\n", aw210xx);

		g_awledstimer = aw210xx;
		leds_work_timer.expires = jiffies + HZ/100;
		timer_setup(&leds_work_timer, leds_breath_func, 0);
		leds_timer_status = 1;
		add_timer(&leds_work_timer);
		INIT_WORK(&ledseffect_work, ledseffect_work_func);

    } else if (val == 0 && leds_timer_status == 1){
        AW_ERR("-------------------------------- DEL TIMER --------------------------------\n");
        leds_timer_status = 0;
		aw210xx_i2c_write(g_awledstimer,AW210XX_REG_BR00L+2*0,    0);//rgb1-b
		aw210xx_i2c_write(g_awledstimer,AW210XX_REG_BR00L+2*1,    0);//rgb1-g
		aw210xx_i2c_write(g_awledstimer,AW210XX_REG_BR00L+2*2,   0);//rgb1-r

		aw210xx_i2c_write(g_awledstimer,AW210XX_REG_BR00L+2*3,  0);//rgb2-r
		aw210xx_i2c_write(g_awledstimer,AW210XX_REG_BR00L+2*4,  0);//rgb2-g
		aw210xx_i2c_write(g_awledstimer,AW210XX_REG_BR00L+2*5,   0);//rgb2-b

		aw210xx_i2c_write(aw210xx,AW210XX_REG_UPDATE,      0x00);//
		del_timer(&leds_work_timer);
		aw210xx_hw_enable(aw210xx, false);
    } else {
 		AW_ERR("-------------------------------- ALWAYS OPEN / ALWAYS OPEN --------------------------------\n");
	}
...
static ssize_t aw210xx_ledswork_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
    ssize_t len = 0;

	len += snprintf(buf + len, PAGE_SIZE - len, "leds_timer_status=%d\n",
		leds_timer_status);

    return len;
}

static ssize_t aw210xx_led_poweroffchg_mode_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
    ssize_t len = 0;

	len += snprintf(buf + len, PAGE_SIZE - len, "led_poweroffchg_mode =%d\n",
		g_leds_poweroffchg_mode);
...
static DEVICE_ATTR(ledwork, 0664, aw210xx_ledswork_show, aw210xx_ledswork_store);
static DEVICE_ATTR(led_poweroffchg_mode, 0664, aw210xx_led_poweroffchg_mode_show, NULL);
...
static struct attribute *aw210xx_attributes[] = {
	&dev_attr_reg.attr,
	&dev_attr_hwen.attr,
	&dev_attr_ledwork.attr,
	&dev_attr_led_poweroffchg_mode.attr,
...
static int aw210xx_read_chipid(struct aw210xx *aw210xx)
{
	...
	while (cnt < AW_READ_CHIPID_RETRIES) {
		...
		if (ret < 0) {
			...
			switch (aw210xx->chipid) {
			case AW21018_CHIPID:
				AW_ERR("AW21018, read chipid = 0x%02x!!\n",
						chipid);
				return 0;
			case AW21012_CHIPID:
				AW_ERR("AW21012, read chipid = 0x%02x!!\n",
						chipid);
				return 0;
			case AW21009_CHIPID:
				AW_ERR("AW21009, read chipid = 0x%02x!!\n",
						chipid);
				return 0;
			default:
		...
		struct device_node *np)
{
	...
	AW_ERR("aw210xx->enable_gpio  numebr %d :\n",aw210xx->enable_gpio );
	aw210xx->sel_gpio = of_get_named_gpio(np, "aw21009-sel-gpio", 0);
    AW_ERR("aw210xx->sel_gpio  numebr %d :\n",aw210xx->sel_gpio );
		...
		const struct i2c_device_id *id)
{
	...
	ssize_t retstat = 0;
	pos = 0;
	pfile = filp_open("proc/bootconfig", O_RDONLY, 0);
	if (IS_ERR(pfile)) {
		AW_ERR("opencmdline file failed!\n");
	} else {
		retstat = kernel_read(pfile, command_line, 1024 , &pos);
		AW_ERR("LED bootconfig line: %s\n", command_line);
		if (strstr(command_line, "androidboot.mode = \"charger\"") != NULL) {
			g_leds_poweroffchg_mode = 1;
			AW_ERR("enter power off mode\n");
		} else {
			AW_ERR("LED command line is not  power off mode \n");
		}
		filp_close(pfile, NULL);
	}

	g_pcb_name = get_type_name();
	AW_ERR("g_pcb_name is %s \n", g_pcb_name);
	if (strstr(g_pcb_name, "EVT0") != NULL) {
		g_is_evt0 = true;
		AW_ERR("This is EVT0 %d \n", g_is_evt0);
	} else {
		g_is_evt0 = false;
		AW_ERR("This is not EVT0 \n");
	}
	...
	if (gpio_is_valid(aw210xx->sel_gpio)) {
		ret = devm_gpio_request_one(&i2c->dev, aw210xx->sel_gpio,
				GPIOF_OUT_INIT_HIGH, "aw210xx_sel");
		if (ret) {
			AW_ERR("sel gpio request failed\n");
		}
	}
	...
	if (ret < 0) {
    	...
    	if (g_leds_poweroffchg_mode == 1) 
	{
		aw210xx_ledswork_start(aw210xx);
	} else {
		aw210xx_hw_enable(aw210xx, false);
	}
