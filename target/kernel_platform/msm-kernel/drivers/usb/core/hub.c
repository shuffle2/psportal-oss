 ...
 * (C) Copyright 2023 Sony Interactive Entertainment Inc.
 *
...
#define HUB_MAJOR_VENDOR   0x1a86
#define HUB_MAJOR_PRODUCT  0X8091
#define HUB_SECOND_VENDOR  0x05e3
#define HUB_SECOND_PRODUCT 0X0608
#define DUALSENSE_VENDOR   0x054c
#define DUALSENSE_PRODUCT  0x0ce6
#define SKYMINOR_VENDOR    0x054c
#define SKYMINOR_PRODUCT   0x0ecc
#define SKYORGIN_VENDOR    0X170d
#define SKYORGIN_PRODUCT   0X0585

struct usb_device *g_usbDev = NULL;

void set_hidraw_state(unsigned long hidraw_event)
{
	usb_notify_add_hub_device(g_usbDev, hidraw_event);
}
EXPORT_SYMBOL_GPL(set_hidraw_state);
...
int usb_device_supports_lpm(struct usb_device *udev)
{
	 ...
	 */
	if (!udev->bos->ss_cap) {
		dev_err(&udev->dev, "No LPM exit latency info found, disabling LPM.\n");
		return 0;
	}
	...
	if (udev->bos->ss_cap->bU1devExitLat == 0 &&
		...
		if (udev->parent)
			dev_err(&udev->dev, "LPM exit latency is zeroed, disabling LPM.\n");
		else
			dev_err(&udev->dev, "We don't know the algorithms for LPM for this host, disabling LPM.\n");
		return 0;
	}
	...
	struct usb_endpoint_descriptor *endpoint)
{
	...
	dev_err(hub_dev, "%d port%s detected\n", maxchild,
			(maxchild == 1) ? "" : "s");
...
static int hub_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
	...
	/* We found a hub */
	dev_err(&intf->dev, "USB hub found\n");
...
static void announce_device(struct usb_device *udev)
{
	...
	u16 deviceVid = le16_to_cpu(udev->descriptor.idVendor);
	u16 devicePid = le16_to_cpu(udev->descriptor.idProduct);
	g_usbDev = udev;
	...
	if (((deviceVid == HUB_MAJOR_VENDOR) && (devicePid == HUB_MAJOR_PRODUCT)) ||
		((deviceVid == HUB_SECOND_VENDOR) && (devicePid = HUB_SECOND_PRODUCT))) {   //hub device
		usb_notify_add_hub_device(udev, USB_HUB_DEVICE);
	} else if ((deviceVid == DUALSENSE_VENDOR) && (devicePid == DUALSENSE_PRODUCT)) {   //cdb device
		usb_notify_add_hub_device(udev, USB_CDB_DEVICE);
	} else if ((deviceVid == SKYMINOR_VENDOR) && (devicePid == SKYMINOR_PRODUCT)) {   //sky device
		usb_notify_add_hub_device(udev, USB_SKY_DEVICE);
	} else if ((deviceVid == SKYORGIN_VENDOR) && (devicePid == SKYORGIN_PRODUCT)) {   //sky device orgin
		usb_notify_add_hub_device(udev, USB_SKY_DEVICE);
	} else {
		//do nothing
	}
...
static int usb_enumerate_device_otg(struct usb_device *udev)
{
	...
	if (!udev->bus->is_b_host
		...
		dev_err(&udev->dev, "Dual-Role OTG device on %sHNP port\n",
...
int usb_authorize_device(struct usb_device *usb_dev)
{
	...
	dev_err(&usb_dev->dev, "authorized to connect\n");
...
check_highspeed(struct usb_hub *hub, struct usb_device *udev, int port1)
{
	...
	if (status == sizeof *qual) {
		dev_err(&udev->dev, "not running at top speed; "
...
loop:
		...
		if (i == (PORT_INIT_TRIES - 1) / 2) {
			dev_err(&port_dev->dev, "attempt power cycle\n");
...
static int usb_reset_and_verify_device(struct usb_device *udev)
{
	...
	if (descriptors_changed(udev, &descriptor, bos)) {
		dev_err(&udev->dev, "device firmware changed\n");
