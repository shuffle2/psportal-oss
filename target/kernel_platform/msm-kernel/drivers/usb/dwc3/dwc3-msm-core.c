...
int hub_mount_state[STATE_MAX] = {0};

void set_usb_mount_state(int vidType, int vidValue)
{
	hub_mount_state[vidType] = vidValue;
}
EXPORT_SYMBOL_GPL(set_usb_mount_state);

int get_usb_mount_state(int vidType)
{
	return hub_mount_state[vidType];
}
EXPORT_SYMBOL_GPL(get_usb_mount_state);
...
void dwc3_restart_usb_work(struct work_struct *w)
{
	...
	pr_err("into dwc3_restart_usb_work func!!!!!!!!!\n");
...
EXPORT_SYMBOL(dwc3_restart_usb_work);
...
static int dwc3_msm_suspend(struct dwc3_msm *mdwc, bool force_power_collapse)
{
	...
	dev_err(mdwc->dev, "DWC3 in low power mode\n");
	...
	unsigned long event, void *ptr)
{
	...
	if (event) {
		dbg_log_string("%s:=======USB_ROLE_HOST========\n", __func__);
		dwc3_msm_set_role(mdwc, USB_ROLE_NONE);
		msleep(150);
		dwc3_msm_set_role(mdwc, USB_ROLE_HOST);
		return NOTIFY_DONE;
	} else if (!event) {
		dwc3_msm_set_role(mdwc, USB_ROLE_NONE);
		msleep(150);
	...
	unsigned long event, void *ptr)
{
	...
	if (event) {
		dbg_log_string("%s:==========USB_ROLE_DEVICE=============\n",__func__);
		dwc3_msm_set_role(mdwc, USB_ROLE_NONE);
		msleep(150);
		dwc3_msm_set_role(mdwc, USB_ROLE_DEVICE);
		return NOTIFY_DONE;
	} else if (!event) {
		dwc3_msm_set_role(mdwc, USB_ROLE_NONE);
		msleep(150);
	...
	unsigned long event, void *ptr)
{
	...
	pr_err("dwc3_msm_host_notifier event = %d\n", event);

	if (event == USB_HUB_DEVICE) {
		set_usb_mount_state(HUB_DEVICE, 1);
		return NOTIFY_DONE;
	} else if (event == USB_CDB_DEVICE) {
		set_usb_mount_state(CDB_DEVICE, 1);
		return NOTIFY_DONE;
	} else if (event == USB_SKY_DEVICE) {
		set_usb_mount_state(SKY_DEVICE, 1);
		return NOTIFY_DONE;
	} else if (event == USB_CDB_HIDRAW) {
		set_usb_mount_state(CDB_HIDRAW, 1);
		return NOTIFY_DONE;
	} else if (event == USB_SKY_HIDRAW) {
		set_usb_mount_state(SKY_HIDRAW, 1);
		return NOTIFY_DONE;
	} else {
		//do nothing
	}
...
static int dwc3_otg_start_host(struct dwc3_msm *mdwc, int on)
{
	...
	if (on) {
		...
		pr_err("dwc3_msm_host_notifier is register!!!!!!!!!!\n");
