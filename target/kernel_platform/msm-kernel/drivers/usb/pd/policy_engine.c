...
static const u32 default_src_caps[] = { 0x19096 };	/* VSafe5V @ 1.5A */
static const u32 default_snk_caps[] = { 0x1912C };	/* VSafe5V @ 3A */
...
static int pd_select_pdo(struct usbpd *pd, int pdo_pos, int uv, int ua)
{
	...
	if (type == PD_SRC_PDO_TYPE_FIXED) {
		...
		pd->rdo = PD_RDO_FIXED(pdo_pos, 0, mismatch, 0, 1, curr / 10,
				max_current / 10);
...
static void pd_force_5V_pdo(struct usbpd *pd)
{
	int voltage = 5000000;  /* force default 5V */
	int i = 0;
	for (i = 0; i < ARRAY_SIZE(pd->received_pdos); i++) {
		if (PD_SRC_PDO_TYPE(pd->received_pdos[i]) == PD_SRC_PDO_TYPE_AUGMENTED) {
			if ((PD_APDO_MAX_VOLT(pd->received_pdos[i]) * 100000) > voltage) { /* 100000: AUGMENTED TYPE convert to voltage */
				pd->received_pdos[i] = 0x00;
			}
		} else if (PD_SRC_PDO_TYPE(pd->received_pdos[i]) == PD_SRC_PDO_TYPE_FIXED) {
			if ((PD_SRC_PDO_FIXED_VOLTAGE(pd->received_pdos[i]) * 50 * 1000) > voltage) { /* 50 * 1000: FIXED TYPE convert to voltage */
				pd->received_pdos[i] = 0x00;
			}
		} else {
			pd->received_pdos[i] = 0x00;
...
static void usbpd_set_state(struct usbpd *pd, enum usbpd_state next_state)
{
	...
	usbpd_dbg(&pd->dev, "set usbpb state: %s -> %s\n",
		usbpd_state_strings[pd->current_state],
...
static void psy_changed_notifier_work(struct work_struct *w)
{
	...
	if (typec_mode >= QTI_POWER_SUPPLY_TYPEC_SINK && typec_mode <=
	    QTI_POWER_SUPPLY_TYPEC_POWERED_CABLE_ONLY) {
		usbpd_err(&pd->dev,"TYPEC MODE:%d",typec_mode);
		return;
	}
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	usbpd_err(&pd->dev,"before:uv = (%d) ua = (%d)  pdo = (%d)\n", uv, ua, pdo);
					...
					struct pd_phy_ops *pdphy_ops)
{
	...
	if (pd->num_sink_caps > 0) {
		...
		pd->sink_caps[0] |= PD_SNK_PDO_FIXED(0, 0, 0, 0, 0, 0, 0);
	} else {
