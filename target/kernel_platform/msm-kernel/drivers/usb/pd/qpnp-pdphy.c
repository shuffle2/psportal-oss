...
int pd_phy_open(struct pd_phy_params *params)
{
	...
	if  (pdphy->usbpd != NULL) {
		...
		pr_err("pd_phy_open don't enable irq!\n");
	}
...
static irqreturn_t pdphy_msg_rx_irq(int irq, void *data)
{
	...
	if (pdphy->msg_rx_cb && (pdphy->usbpd != NULL))
...
static int pdphy_probe(struct platform_device *pdev)
{
	...
	if (pdphy->is_opened == true) {
		pr_err("pdphy_probe redo enable irq!\n");
		pdphy_enable_irq(pdphy, true);
	}

	pdphy_create_debugfs_entries(pdphy);

	pr_err("pdphy_probe are successfully!\n");
	return 0;
}
