...
#include"../../../input/touchscreen/focaltech_touch/focaltech_core.h"
#include "../../../misc/sx937x/sx937x.h"

#if defined(FACTORY_VERSION)
static  int factory_version = 1;
#else
static  int factory_version = 0;
#endif
...
static bool pogo_device = true;
static bool status_gpio66 = false;
static bool status_chg = false;
static int g_count = 0;
extern void tp_switch_chage_mod(bool mode);
extern void sensor_chage_mod(bool mode);
void extcon_get_charge_mode(bool charge_mode);
static void smblib_handle_rp_change(struct smb_charger *chg, int typec_mode);
static bool smblib_rsbux_low(struct smb_charger *chg, int r_thr);
static int moisture_detect_delay = 0; // moisture detect delay 60s
			    ...
			    struct smb_chg_param *param, int val_u)
{
	...
	if (param->set_proc) {
		...
		if ((param->reg == USBIN_CURRENT_LIMIT_CFG_REG) && (val_raw >= 2))  {
			val_raw -= 1;
			smblib_dbg(chg, PR_REGISTER, "workaround for USB Compliance: reduce USB_ICL one step.");
		}
	}
					...
					u8 allowed_voltage)
{
	...
	smblib_err(chg, "set USBIN_ALLOW_OVERRIDE: %d\n",
			allowed_voltage);
	return rc;
}
...
int smblib_set_icl_current(struct smb_charger *chg, int icl_ua)
{
	...
	pr_err("smblib_set_icl_current icl_ua=%d.\n", icl_ua);
#else
	static int g_icl_ua = 0;
	if (g_icl_ua != icl_ua) {
		pr_err("smblib_set_icl_current icl_ua=%d.\n", icl_ua);
		g_icl_ua = icl_ua;
	}
	...
	if ((icl_ua == 100000) && (pval.intval == 1)) {
		icl_ua = 500000;
	}
...
int smblib_vconn_regulator_disable(struct regulator_dev *rdev)
{
	...
	smblib_err(chg, "disabling VCONN\n");
	rc = smblib_masked_write(chg, TYPE_C_VCONN_CONTROL_REG,
...
int smblib_run_aicl(struct smb_charger *chg, int type)
{
	...
	pr_err("smblib_run_aicl %s AICL\n", (type == RERUN_AICL) ? "RERUN" : "RESTART");
...
static int smblib_set_chg_temp_regulation(struct smb_charger *chg, bool enable)
{
	int rc = 0;
	...
	if (enable) {
		status_chg = true;
		if (!status_gpio66) {
			schedule_delayed_work(&chg->thermal_regulation_work, msecs_to_jiffies(1000));
			pr_err("Schedule work !!!\n");
		}
	}
	else if (!enable) {
		status_chg = false;
...
#if OPEN_CHARGER_USER_LOG
static void smblib_pintf_work()
{
	struct power_supply *psy;
	union power_supply_propval socvalue;
	union power_supply_propval volvalue;
	union power_supply_propval curvalue;
	union power_supply_propval tempvalue;
	union power_supply_propval ibusvalue;
	union power_supply_propval vbusvalue;
	psy = power_supply_get_by_name("battery");
	power_supply_get_property(psy, POWER_SUPPLY_PROP_CAPACITY, &socvalue);
	power_supply_get_property(psy, POWER_SUPPLY_PROP_VOLTAGE_NOW, &volvalue);
	power_supply_get_property(psy, POWER_SUPPLY_PROP_CURRENT_NOW, &curvalue);
	power_supply_get_property(psy, POWER_SUPPLY_PROP_TEMP, &tempvalue);
	psy = power_supply_get_by_name("usb");
	power_supply_get_property(psy, POWER_SUPPLY_PROP_CURRENT_NOW, &ibusvalue);
	power_supply_get_property(psy, POWER_SUPPLY_PROP_VOLTAGE_NOW, &vbusvalue);
    pr_err("healthd -printf battery level = %d  voltage = %d  current = %d  temp = %d  ibusvalue = %d  vbusvalue = %d\n",
	socvalue.intval,volvalue.intval,curvalue.intval,tempvalue.intval,ibusvalue.intval,vbusvalue.intval);
}
#endif
...
#define SKIN_TEMP_SHDN_THRESH		800
...
#define MOISTURE_DETECT_DELAY  60 // moisture detect delay 60s
...
#if OPEN_CHARGER_USER_LOG
	pr_err("process thermal connector_temp = %d, skin_temp = %d\n",chg->connector_temp, chg->skin_temp);
	smblib_pintf_work();
#endif

	smblib_read(chg, BATTERY_CHARGER_STATUS_2_REG, &stat);
	if (chg->chg_input_dev != NULL) {
		if (stat & CHARGER_ERROR_CHG_TIME_OUT_BIT) {
			input_report_key(chg->chg_input_dev, KEY_CHG_TIMEOUT, 1);
			input_sync(chg->chg_input_dev);
		} else {
			input_report_key(chg->chg_input_dev, KEY_CHG_TIMEOUT, 0);
			input_sync(chg->chg_input_dev);
		}
	}

	if ((chg->moisture_present == false) && smblib_rsbux_low(chg, 3000000)) {
		pr_err("smblib_rsbux_low true\n");
		chg->moisture_present = true;
		moisture_detect_delay = 0;
		/* Moisture detected, enable sink only mode */
		vote(chg->usb_icl_votable, "user_moisture_limit", true, 0);
		if ((chg->chg_input_dev != NULL) && (status_chg)) {
			input_report_key(chg->chg_input_dev, KEY_MOISTURE_USB, 1);
			input_sync(chg->chg_input_dev);
        	}
	}  else if  ((moisture_detect_delay >= MOISTURE_DETECT_DELAY) &&
		(chg->moisture_present == true) && !smblib_rsbux_low(chg, 3000000)) {
		pr_err("smblib_rsbux_low false\n");
		chg->moisture_present = false;
		moisture_detect_delay = 0;
		vote(chg->usb_icl_votable, "user_moisture_limit", false, 0);
		if (chg->chg_input_dev != NULL) {
			input_report_key(chg->chg_input_dev, KEY_MOISTURE_USB, 0);
			input_sync(chg->chg_input_dev);
        	}
	} else {
		if (chg->moisture_present == true) {
			if (moisture_detect_delay >= MOISTURE_DETECT_DELAY) {
				moisture_detect_delay = 1;
			} else {
				moisture_detect_delay++;
			}
		}
	}
...
out:
	...
	if (thermal_status != chg->thermal_status) {
		...
		vote(chg->usb_icl_votable, SW_THERM_REGULATION_VOTER,
							suspend_input, 0);
		if (suspend_input == true) {
			pinctrl_select_state(chg->pinctrl, chg->pin_therm_active);
			status_gpio66 = true;
			if (chg->chg_input_dev != NULL) {
				input_report_key(chg->chg_input_dev, KEY_THERMAL_USB, 1);
				input_sync(chg->chg_input_dev);
			}
#if !OPEN_CHARGER_USER_LOG
			pr_err("process thermal connector_temp = %d, skin_temp = %d\n",chg->connector_temp, chg->skin_temp);
#endif
			pr_err("pin therm default: GPIO_66 output-high, temperature too high\n");
		} else if(thermal_status != TEMP_ALERT_LEVEL) {
			pinctrl_select_state(chg->pinctrl, chg->pin_therm_default);
			status_gpio66 = false;
			if (chg->chg_input_dev != NULL) {
				input_report_key(chg->chg_input_dev, KEY_THERMAL_USB, 0);
				input_sync(chg->chg_input_dev);
			}
#if !OPEN_CHARGER_USER_LOG
			pr_err("process thermal connector_temp = %d, skin_temp = %d\n",chg->connector_temp, chg->skin_temp);
#endif
			pr_err("pin therm default: GPIO_66 output-low, temperature normal\n");
		}
		smblib_err(chg, "USB/DC %s as per SW thermal regulation\n",
				    ...
				    int val)
{
	...
	if (pogo_device) {
		pr_err("smblib_set_prop_sdp_current_max exit with pogo device.\n");
		return rc;
	}
...
static int smb5_configure_recharging(struct smb_charger *chg, int recharg_vbat_threholds)
{
	int rc = 0;
	u32 temp = 0;
	...
	temp = VBAT_TO_VRAW_ADC(recharg_vbat_threholds);
	temp = ((temp & 0xFF00) >> 8) | ((temp & 0xFF) << 8);
...
static int smblib_soft_jeita_arb_wa(struct smb_charger *chg)
{
	...
	if (pval.intval == POWER_SUPPLY_HEALTH_COLD ||
		...
		int rc = 0;
		u32 jeita_hard_hys_thlds[2] = {12678,3290}; /* COLD = 3 degC, HOT = 52 degC */ 
		rc = smblib_update_jeita(chg, jeita_hard_hys_thlds,
					JEITA_HARD);
		if (rc < 0)
			smblib_err(chg,
	...
	if (soft_jeita) {
		int rc = 0;
		u32 jeita_hard_hys_thlds[2] = {13710,3056}; /* COLD = 0 degC, HOT = 55 degC */ 
		rc = smblib_update_jeita(chg, jeita_hard_hys_thlds,
					JEITA_HARD);
		if (rc < 0)
			smblib_err(chg,
...
unsuspend_input:
		...
		if (chg->aicl_max_reached) {
			pr_err("Reached max AICL threshold resctricting ICL to 100mA\n");
			smblib_dbg(chg, PR_MISC,
...
static void smblib_usb_plugin_locked(struct smb_charger *chg)
{
	...
	pr_err("smblib_usb_plugin_locked charger type changed, set gpio75 low!!!\n");
	...
	if (vbus_rising) {
		...
		/* Enable CHG temp regulation */
		rc = smblib_set_chg_temp_regulation(chg, true);
		if (rc < 0)
			smblib_err(chg, "Couldn't start CHG temp regulation, rc=%d\n", rc);
		...
		/* Disable CHG temp regulation */
		rc = smblib_set_chg_temp_regulation(chg, false);
		if (rc < 0)
			smblib_err(chg, "Couldn't stop CHG temp regulation, rc=%d\n", rc);
	...
	smblib_err(chg, "IRQ: usbin-plugin %s\n",
					vbus_rising ? "attached" : "detached");
        tp_switch_chage_mod(vbus_rising);
}
...
static void update_sw_icl_max(struct smb_charger *chg, int val)
{
	...
	int rp_ua;
	union power_supply_propval pval;
	if ((val == POWER_SUPPLY_TYPE_USB) || (val == POWER_SUPPLY_TYPE_USB_CDP)) {
...
static void smblib_rerun_apsd_work(struct work_struct *work)
{
	...
	struct smb_irq_data *data = NULL;
	int typec_mode;
	const struct apsd_result *apsd_result = smblib_get_apsd_result(chg);

	data = chg->irq_info[TYPEC_ATTACH_DETACH_IRQ].irq_data;
	if (apsd_result->val == POWER_SUPPLY_TYPE_USB_DCP) {
		smb5_typec_attach_detach_irq_handler(0, data);
		typec_mode = smblib_get_prop_typec_mode(chg);
		...
		power_supply_changed(chg->usb_psy);
		pr_err("get cc-state-change again\n");
	}

	msleep(1000); /* wait for pd detect */
	if ((chg->pd_active == 0) && (apsd_result->val != POWER_SUPPLY_TYPE_USB) && (apsd_result->val != POWER_SUPPLY_TYPE_USB_CDP)) {
		pr_err("force rerun apsd\n");
		smblib_rerun_apsd_if_required(chg);
		apsd_result = smblib_get_apsd_result(chg);
	}
...
static void smblib_handle_rp_change(struct smb_charger *chg, int typec_mode)
{
	...
	smblib_err(chg, "CC change old_mode=%d new_mode=%d\n",
						chg->typec_mode, typec_mode);
}
...
irqreturn_t smb5_dc_plugin_irq_handler(int irq, void *data)
{
	...
	smblib_err(chg, "dcin_present= %d, usbin_present= %d, cp_reason = %d\n",
...
static void smblib_chg_termination_work(struct work_struct *work)
{
	...
	smblib_err(chg, "Chg Term WA readings: cc_soc: %d, cc_soc_ref: %d, delay: %d vbat_now %d term_vbat %d\n",
...
static void jeita_update_work(struct work_struct *work)
{
	...
	pr_err("jeita_update_work: chg->jeita_configured = %d\n", chg->jeita_configured);
out:
...
static int extcon_input_chg_error_init(struct smb_charger *chg)
{
	int ret = 0;
	chg->chg_input_dev = devm_input_allocate_device(chg->dev);
	if (!chg->chg_input_dev) {
		pr_err("Failed to allocate memory for is-chg-error device");
		return -ENOMEM;
	}
	chg->chg_input_dev->name = "is-chg-error";
	chg->chg_input_dev->phys = "/is_chg_error/input0";
	chg->chg_input_dev->evbit[0] = BIT_MASK(EV_KEY);
	input_set_capability(chg->chg_input_dev, EV_KEY, KEY_THERMAL_USB);
	input_set_capability(chg->chg_input_dev, EV_KEY, KEY_MOISTURE_USB);
	input_set_capability(chg->chg_input_dev, EV_KEY, KEY_CHG_TIMEOUT);
	ret = input_register_device(chg->chg_input_dev);
	if (ret) {
		pr_err("Input device registration failed");
		input_free_device(chg->chg_input_dev);
		chg->chg_input_dev = NULL;
		return ret;
	}
	...
	input_report_key(chg->chg_input_dev, KEY_CHG_TIMEOUT, 0);
	input_sync(chg->chg_input_dev);
	pr_err("extcon_input_chg_error_init Success !\n");
	return ret;
}

int pinctrl_therm_init(struct smb_charger *chg)
{
	int ret = 0;

	struct device *dev = chg->dev;
	if (chg == NULL) {
		pr_err("charger therm pinctrl_therm_init data is NULL\n");
		return -1;
	}
	chg->pinctrl = devm_pinctrl_get(dev);
	if (IS_ERR_OR_NULL(chg->pinctrl)) {
		pr_err("charger therm error: devm_pinctrl_get wrong\n");
		return -1;
	}
	chg->pin_therm_default = pinctrl_lookup_state(chg->pinctrl, "therm_default");
	if (IS_ERR_OR_NULL(chg->pin_therm_default)) {
		pr_err("charger therm default error: pinctrl_lookup_state wrong\n");
		return -1;
	}
	chg->pin_therm_active = pinctrl_lookup_state(chg->pinctrl, "therm_active");
	if (IS_ERR_OR_NULL(chg->pin_therm_active)) {
		pr_err("charger therm active error: pinctrl_lookup_state wrong\n");
		return -1;
	}
	ret = pinctrl_select_state(chg->pinctrl, chg->pin_therm_default);
	if (ret < 0) {
		pr_err("charger therm default error: pinctrl_select_state_to_default wrong !\n");
	}
	pr_err("GPIO_66 init set default success!\n");

	ret = extcon_input_chg_error_init(chg);
...
int smblib_init(struct smb_charger *chg)
{
	...
	rc = pinctrl_therm_init(chg);
	if (rc < 0) {
		pr_err("Couldn't init pinctrl therm rc=%d\n", rc);
	}
