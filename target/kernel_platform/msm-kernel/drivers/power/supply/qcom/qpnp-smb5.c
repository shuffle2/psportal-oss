		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_AUTHENTIC:
		smb5_iio_get_prop(chg, PSY_IIO_TYPEC_CC_ORIENTATION, &val->intval);
		pr_err("cc_orientation = %d\n", val->intval);
	break;
	default:
...
static enum power_supply_usb_type smb5_usb_type[] = {
...
static const struct power_supply_desc usb_psy_desc = {
	...
	.usb_types = smb5_usb_type,
	.num_usb_types = ARRAY_SIZE(smb5_usb_type),
		...
		union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_INPUT_POWER_LIMIT:
		...
		pval->intval = smblib_read(chg, SHIP_MODE_REG, &value);
		break;
	...
	case POWER_SUPPLY_PROP_CALIBRATE:
		rc = smblib_get_prop_from_bms(chg,
				SMB5_QG_RESISTANCE_ID, &pval->intval);
		break;
	default:
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_INPUT_POWER_LIMIT:
		...
		if (!val->intval)
			break;
		rc = smblib_set_prop_ship_mode(chg, val->intval);
		pr_err("smb5 set prop ship mode: %d\n", val->intval);
		break;
	default:
		rc = -EINVAL;
	}
...
static int smb5_configure_typec(struct smb_charger *chg)
{
	...
	pr_err("smb5 configure typec ok.\n");
	return rc;
}
...
static int smb5_show_charger_status(struct smb5 *chip)
{
	...
	pr_err("SMB5 status - usb:present=%d type=%d batt:present = %d health = %d charge = %d\n",
