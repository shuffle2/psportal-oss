...
static void qg_notify_charger(struct qpnp_qg *chip)
{
	...
	pr_err("qg_notify_charger prop.intval = %d\n", prop.intval);
}
...
static void process_udata_work(struct work_struct *work)
{
	...
	rc = get_batt_id_ohm(chip, &batt_id_N);
	if (chip->qg_input_dev != NULL) {
		if (batt_id_N < 1000) {
			input_report_key(chip->qg_input_dev, KEY_BAT_SHORT_CUT, 1);
			input_sync(chip->qg_input_dev);
		} else {
			input_report_key(chip->qg_input_dev, KEY_BAT_SHORT_CUT, 0);
			input_sync(chip->qg_input_dev);
		}
	}
...
static irqreturn_t qg_vbat_low_handler(int irq, void *data)
{
	...
	qg_dbg(chip, QG_DEBUG_IRQ, "qg vbat low:IRQ triggered\n");
	mutex_lock(&chip->data_lock);
...
static irqreturn_t qg_vbat_empty_handler(int irq, void *data)
{
	...
	qg_dbg(chip, QG_DEBUG_IRQ, "qg vbat handler:IRQ triggered\n");
...
static irqreturn_t qg_good_ocv_handler(int irq, void *data)
{
	...
	qg_dbg(chip, QG_DEBUG_IRQ, "qg good ocv:IRQ triggered\n");
...
static int save_battery_health(struct qpnp_qg *chip)
{
	union power_supply_propval prop = {0, };
	int rc;
...
static int qg_charge_full_update(struct qpnp_qg *chip)
{
	...
	if (chip->charge_done && !chip->charge_full) {
		if (chip->msoc >= 99 && health == POWER_SUPPLY_HEALTH_GOOD) {
			...
			qg_err(chip, QG_DEBUG_STATUS, "Setting charge_full (0->1) @ msoc=%d\n",
					chip->msoc);
		} else if (health != POWER_SUPPLY_HEALTH_GOOD) {
			/* terminated in JEITA */
			qg_err(chip, QG_DEBUG_STATUS, "Terminated charging @ msoc=%d\n",
					chip->msoc);
		}
		...
		if (chip->msoc < recharge_soc || !input_present) {
			...
			qg_err(chip, QG_DEBUG_STATUS, "msoc=%d recharge_soc=%d charge_full (1->0)\n",
					chip->msoc, recharge_soc);
		} else {
			/* continue with charge_full state */
			qg_err(chip, QG_DEBUG_STATUS, "msoc=%d recharge_soc=%d charge_full=%d input_present=%d\n",
					chip->msoc, recharge_soc,
...
static void qg_status_change_work(struct work_struct *work)
{
	...
	qg_err(chip, QG_DEBUG_STATUS, "charge_status=%d charge_done=%d\n",
...
static int qg_register_device(struct qpnp_qg *chip)
{
	...
	qg_err(chip, QG_DEBUG_DEVICE, "'/dev/qg' successfully created\n");
...
static int qg_setup_battery(struct qpnp_qg *chip)
{
	...
	qg_err(chip, QG_DEBUG_PROFILE, "battery_missing=%d batt_id_ohm=%d Ohm profile_loaded=%d profile=%s\n",
...
{
	int rc = 0, batt_temp = 0, i, shutdown_temp = 0, batt_voltage = 0;
...
use_pon_ocv:
	if (use_pon_ocv) {
		...
		qg_err(chip, QG_DEBUG_PON, "v_float=%d v_cutoff=%d FULL_SOC=%d CUTOFF_SOC=%d PON_SYS_SOC=%d pon_soc=%d\n",
	...
	rc = qg_get_battery_voltage(chip, &batt_voltage);
	pr_err("battery voltage = %d,    boot_soc = %d\n", batt_voltage, soc);
		...
		struct device_node *node, const char *irq_name)
{
	...
	qg_err(chip, QG_DEBUG_PON, "IRQ %s registered wakeable=%d\n",
...
#define DEFAULT_SHUTDOWN_SOC_SECS	259200 /* 3 x 24 hours */
#define DEFAULT_COLD_TEMP_THRESHOLD	0
#define DEFAULT_SHUTDOWN_TEMP_DIFF	200	/* 20 degC */
...
static int qg_parse_dt(struct qpnp_qg *chip)
{
	...
	qg_err(chip, QG_DEBUG_PON, "DT: vbatt_empty_mv=%dmV vbatt_low_mv=%dmV delta_soc=%d ext-sns=%d\n",
...
static int event_batt_id_init(struct qpnp_qg *chip)
{
	int ret = 0;
	chip->qg_input_dev = devm_input_allocate_device(chip->dev);
	if (!chip->qg_input_dev) {
        	pr_err("Failed to allocate memory for chip->qg device\n");
		return -ENOMEM;
	}
	chip->qg_input_dev->name = "check-batt_id";
	chip->qg_input_dev->phys = "/check_batt_id/input0";
	chip->qg_input_dev->evbit[0] = BIT_MASK(EV_KEY);
	input_set_capability(chip->qg_input_dev, EV_KEY, KEY_BAT_SHORT_CUT);
	ret = input_register_device(chip->qg_input_dev);
	if (ret) {
		pr_err("Input device registration failed\n");
		input_free_device(chip->qg_input_dev);
		chip->qg_input_dev = NULL;
		return ret;
	}
...
static int qpnp_qg_probe(struct platform_device *pdev)
{
	...
	if(!alarmtimer_get_rtcdev()) {
		pr_err("Failed to get alarm-timer, need wait for rtc and retry!\n");
		return -EPROBE_DEFER;
	}
	...
	rc = event_batt_id_init(chip);
	if (rc < 0) {
		pr_err("Failed to init event_batt_id_check rc=%d\n", rc);
	}
	...
	pr_err("QG initialized! battery_profile=%s SOC=%d QG_subtype=%d QG_version=%s QG_mode=%s\n",
