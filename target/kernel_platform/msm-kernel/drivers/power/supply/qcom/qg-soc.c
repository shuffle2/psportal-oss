...
static void update_msoc(struct qpnp_qg *chip)
{
	...
	bool input_present = is_input_present(chip);
	bool h_temp_soc_increase = true;
	struct power_supply *psy;
	union power_supply_propval value;
	union power_supply_propval input_limit;
	int count;  /* for adjust low temp sys soc */
	int adjust_list[N_ADJUST][N_RANK] = {{100, 9820}, {99, 9730}, {98, 9640}, {97, 9550}, {96, 9462}, {95, 9376}, {94, 9290}, {93, 9205}, {92, 9120}, {91, 9035}};

	psy = power_supply_get_by_name("usb");
	power_supply_get_property(psy, POWER_SUPPLY_PROP_USB_TYPE, &value);
	psy = power_supply_get_by_name("battery");
	power_supply_get_property(psy, POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT, &input_limit);

	if ((user_batt_health == POWER_SUPPLY_HEALTH_WARM) && (chip->charge_status != POWER_SUPPLY_STATUS_CHARGING)) {
		h_temp_soc_increase = false;
	}

	if (chip->catch_up_soc > chip->msoc) {
		/* SOC increased */
		if (input_present && (input_limit.intval == 0) && h_temp_soc_increase)/* Increment if input is present */
			chip->msoc += chip->dt.delta_soc;
	} else if (chip->catch_up_soc < chip->msoc) {
		/* SOC dropped */
		if ((!input_present) || (value.intval != POWER_SUPPLY_USB_TYPE_PD &&  
		    value.intval != POWER_SUPPLY_USB_TYPE_DCP) || (input_limit.intval == 1)) {

			if (user_batt_health == POWER_SUPPLY_HEALTH_COOL) {
				pr_err("Adjust Soc in COOL\n");
				for(count = 0; count < N_ADJUST; count++) {
					if (chip->sys_soc > adjust_list[count][1]) {
						chip->msoc = adjust_list[count][0];
						break;
					}
				}
				if (count >= N_ADJUST) {
...
int qg_scale_soc(struct qpnp_qg *chip, bool force_soc)
{
	...
	pr_err("SOC scale: msoc=%d catch_up_soc=%d delta_soc=%d next_wakeup=%d sec\n",
			chip->msoc, chip->catch_up_soc, chip->dt.delta_soc,
			chip->next_wakeup_ms / 1000);
