...
static int __init uart_log_status_setup(char *str)
{
	u64 size;
	pr_info("%s  uart_log_status_setup  \n", str);
	if (!str)
		return -EINVAL;
	size = memparse(str, &str);
	g_uart_status = size;
	pr_info("%d  uart_log_status_setup   size: %d\n", g_uart_status, size);
	return 0;
}
early_param("uart_enable", uart_log_status_setup);

static int __init uart_enable_setup(char *str)
{
	g_uart_status = 1;
	pr_info("uart_log_status_setup  status :%d\n", g_uart_status);
	return 0;
}
__setup("uart_enable=", uart_enable_setup);
