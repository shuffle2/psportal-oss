...
static int mmc_decode_cid(struct mmc_card *card)
{
	...
	sprintf(emmc_cid, "%x", card->cid.manfid);
	for (i = 0; i < ARRAY_SIZE(vendor_table); i++) {

		if (memcmp(emmc_cid, vendor_table[i].id,EMMC_VENDOR_CMP_SIZE) == 0) {
			memcpy(card->cid.manfidname, vendor_table[i].name,strlen(vendor_table[i].name));
		}
	}
	return 0;
}
