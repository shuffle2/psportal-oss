...
int usb_choose_configuration(struct usb_device *udev)
{
	...
	if (best) {
		if (udev->descriptor.idProduct == USB_PRODUCT_ID_WIRELESS_ADAPTER
			&& udev->descriptor.idVendor == USB_VENDOR_ID_SONY ) {
			/* Set configuration 2 as default for PSW adapter.Need keep configurations
                         * order unchanged in the future FW release of PSW adapter. */
			if (udev->descriptor.bNumConfigurations >= 2) {
				best =  udev->config + 1;
			}
		}
		i = best->desc.bConfigurationValue;
		dev_info(&udev->dev,
			"configuration #%d chosen from %d choice%s vid 0x%x pid 0x%x\n",
			i, num_configs, plural(num_configs), udev->descriptor.idVendor,
